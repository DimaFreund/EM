...
class Spidermag_About {
	...
	*/
	public function enqueue_styles() {
		global $spidermag_version;
		wp_enqueue_style( 'spidermag-about-theme', get_template_directory_uri() . '/spidermag/admin/about-theme/about.css', array(), $spidermag_version );
	}
	...
	*/
	public function admin_notice() {
		global $spidermag_version, $pagenow;
		// Let's bail on theme activation.
		if ( 'themes.php' == $pagenow && isset( $_GET['activated'] ) ) {
			add_action( 'admin_notices', array( $this, 'welcome_notice' ) );
			update_option( 'spidermag_admin_notice_welcome', 1 );

		// No option? Let run the notice wizard again..
		} elseif( ! get_option( 'spidermag_admin_notice_welcome' ) ) {
			add_action( 'admin_notices', array( $this, 'welcome_notice' ) );
		}
	}
	...
	public static function hide_notices() {
		if ( isset( $_GET['spidermag-hide-notice'] ) && isset( $_GET['_spidermag_notice_nonce'] ) ) {
			if ( ! wp_verify_nonce( wp_unslash( $_GET['_spidermag_notice_nonce'] ), 'spidermag_hide_notices_nonce' ) ) {
			...
			update_option( 'spidermag_admin_notice_' . $hide_notice, 1 );
		}
	}
	...
	public function welcome_notice() {
		...
		<div id="message" class="updated spidermag-message">
			<a class="spidermag-message-close notice-dismiss" href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'activated' ), add_query_arg( 'spidermag-hide-notice', 'welcome' ) ), 'spidermag_hide_notices_nonce', '_spidermag_notice_nonce' ) ); ?>"><?php esc_html_e( 'Dismiss', 'spidermag' ); ?></a>
			<p><?php printf( esc_html__( 'Welcome! Thank you for choosing spidermag! To fully take advantage of the best our theme can offer please make sure you visit our %1$swelcome page%2$s.', 'spidermag' ), '<a href="' . esc_url( admin_url( 'themes.php?page=spidermag-welcome' ) ) . '">', '</a>' ); ?></p>
			<p class="submit">
				<a class="button-secondary" href="<?php echo esc_url( admin_url( 'themes.php?page=spidermag-welcome' ) ); ?>"><?php esc_html_e( 'Get started with SpiderMag', 'spidermag' ); ?></a>
			</p>
		</div>
		<?php
	}
	...
	private function intro() {
		...
		<p class="spidermag-actions">		
			...
			<a href="<?php echo esc_url( apply_filters( 'spidermag_pro_theme_url', 'https://www.sparklewpthemes.com/wordpress-themes/spidermagpro' ) ); ?>" class="button button-primary docs" target="_blank"><?php esc_html_e( 'View PRO version', 'spidermag' ); ?></a>

			<a href="<?php echo esc_url( apply_filters( 'spidermag_pro_theme_url', 'http://demo.sparklewpthemes.com/spidermagpro/demos/' ) ); ?>" class="button button-primary docs" target="_blank"><?php esc_html_e( 'View PRO Demo', 'spidermag' ); ?></a>
			...
			<a href="<?php echo esc_url( apply_filters( 'spidermag_pro_theme_url', 'http://demo.sparklewpthemes.com/spidermag/demos/' ) ); ?>" class="button button-secondary docs" target="_blank"><?php esc_html_e( 'View Demo', 'spidermag' ); ?></a>


			<a href="<?php echo esc_url( apply_filters( 'spidermag_pro_theme_url', 'https://wordpress.org/support/theme/spidermag/reviews/?filter=5' ) ); ?>" class="button button-secondary docs" target="_blank"><?php esc_html_e( 'Rate this theme', 'spidermag' ); ?></a>
		</p>
	...
	public function about_screen() {
		...
		<div class="wrap about-wrap">
			...
			<div class="return-to-dashboard spidermag">
			...
			<div class="theme-browser rendered">
				<div class="themes wp-clearfix">
					<?php
						...
						$themes = $this->spidermag_get_themes( $request );
						...
						foreach ( $themes->themes as $theme ) {
							if( $active_theme != $theme->name ) {
								...
								$theme_details = $this->spidermag_get_themes( $request );
							?>
...
<?php
	 ...
	 */
	private function spidermag_get_themes( $request ) {

		// Generate a cache key that would hold the response for this request:
		$key = 'spidermag_' . md5( serialize( $request ) );
	...
	public function changelog_screen() {
		...
		<div class="wrap about-wrap">
			...
			<?php
				$changelog_file = apply_filters( 'spidermag_changelog_file', get_template_directory() . '/readme.txt' );
